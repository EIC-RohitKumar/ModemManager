...
static void
function_to_check_at_command_execution   (MMBaseModem  *self,
                                               GAsyncResult *res,
                                               GTask        *task)
{
    GError                *error = NULL;
    ...
    ctx = (SetInitialEpsContext *) g_task_get_task_data (task);
    if (!mm_base_modem_at_command_finish (self, res, &error)) {
        mm_obj_warn (self, "Error executing AT command: %s", error->message);
        g_task_return_error (task, error);
        g_object_unref (task);
        return;
    }

    ctx->step++;     //increment to step value
    set_initial_eps_step (task);   //go to that function
}
...
set_initial_eps_step (GTask *task)
{
    ...
    case SET_INITIAL_EPS_STEP_SET_SETTINGS: {
        ...
        const gint          tmp_cid=1;    //TODO: instead of using default value, cid value will be assigned in runtime implementation

        self->priv->initial_eps_bearer_cid = tmp_cid;
...
iface_modem_3gpp_init (MMIfaceModem3gpp *iface)
{
    ...
    /* Additional steps */
    /* TODO: Following uncommented lines are needed for loadtime implementation */
