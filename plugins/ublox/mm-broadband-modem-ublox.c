...
struct _MMBroadbandModemUbloxPrivate {
    ...
    /* Allowed authentication methods for modem*/
...
/*****************************************************************************/
/* Support for EPS bearer authentication using AT+UAUTHREQ */

gchar *
mm_ublox_build_auth_string (GTask              *task,
                            MMBearerProperties *properties)
{
    ...
    auth_required = (user && password && allowed_auth != MM_BEARER_ALLOWED_AUTH_NONE);
...
out:
    ...
    if (ublox_auth > 0) {
        ...
        cmd = g_strdup_printf ("+UAUTHREQ=%u,0,\"\",\"\"", self->priv->initial_eps_bearer_cid);
